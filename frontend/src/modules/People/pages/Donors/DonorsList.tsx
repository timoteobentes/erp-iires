...
interface Donor {
  ...
  email: string;
  type: 'PF' | 'PJ';
  recurrence: 'Mensal' | 'Única' | 'Anual';
  status: 'active' | 'inactive';
  totalDonated: number;
  lastDonation: string;
}

const mockData: Donor[] = [
  { id: '1', name: 'TechCorp Brasil', email: 'parcerias@techcorp.com.br', type: 'PJ', recurrence: 'Mensal', status: 'active', totalDonated: 120000, lastDonation: '10/04/2026' },
  { id: '2', name: 'Marina Silva', email: 'marina@email.com', type: 'PF', recurrence: 'Mensal', status: 'active', totalDonated: 1200, lastDonation: '15/04/2026' },
  { id: '3', name: 'João Souza', email: 'joao@email.com', type: 'PF', recurrence: 'Única', status: 'inactive', totalDonated: 500, lastDonation: '20/12/2025' },
  { id: '4', name: 'Instituto Inovar', email: 'contato@inovar.org', type: 'PJ', recurrence: 'Anual', status: 'active', totalDonated: 45000, lastDonation: '05/01/2026' },
];
...
export default function DonorsList() {
  ...
  // Modal de Exclusão Padrão Enterprise
  const handleDelete = (id: string, name: string) => {
    Modal.confirm({
      title: 'Inativar Doador',
      content: (
        <div>
          Tem certeza que deseja inativar o doador <strong className="text-dark-900">{name}</strong>?<br/>
          O histórico financeiro de doações passadas será mantido, mas cobranças recorrentes futuras poderão ser suspensas.
        </div>
      ),
      okText: 'Sim, inativar',
      okType: 'danger',
      cancelText: 'Cancelar',
      onOk: () => {
        console.log('Doador inativado:', id);
      },
    });
  };
  ...
  const columns: ColumnsType<Donor> = [
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-3">
          <Avatar 
            src={`https://api.dicebear.com/7.x/initials/svg?seed=${record.name}&backgroundColor=0047AF`} 
    ...
    },
    {
      ...
      key: 'recurrence',
      render: (recurrence) => (
        <Tag className={`rounded-md font-bold px-2 uppercase tracking-wide text-[10px] ${
          recurrence === 'Mensal' ? 'bg-primary-50 text-primary-600 border-none' : 
          recurrence === 'Anual' ? 'bg-secondary-50 text-secondary-600 border-none' : 
      ...
      render: (_, record) => (
        <Space size="small">
          <Tooltip title="Ver Perfil">
            <Button type="text" icon={<Eye size={18} />} onClick={() => navigate(`/people/donors/${record.id}`)} className="text-dark-400 hover:text-secondary-500 transition-colors" />
          </Tooltip>
          <Tooltip title="Editar">
            <Button type="text" icon={<Edit size={18} />} onClick={() => navigate(`/people/donors/${record.id}/edit`)} className="text-dark-400 hover:text-primary-500 transition-colors" />
          </Tooltip>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Controles: Busca e Filtros Avançados */}
         ...
         <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex w-full md:w-auto gap-3 items-center">
            ...
            <div className="hidden sm:flex bg-dark-50 p-1 rounded-lg border border-dark-100 ml-2">
              <Button type="text" className={`p-1 h-auto w-auto rounded-md transition-all ${viewMode === 'list' ? 'bg-white shadow-sm text-dark-900' : 'text-dark-400 hover:text-dark-600'}`} onClick={() => setViewMode('list')}><ListIcon size={18} /></Button>
              <Button type="text" className={`p-1 h-auto w-auto rounded-md transition-all ${viewMode === 'grid' ? 'bg-white shadow-sm text-dark-900' : 'text-dark-400 hover:text-dark-600'}`} onClick={() => setViewMode('grid')}><LayoutGrid size={18} /></Button>
            </div>
          </div>
        </div>
        ...
        {showFilters && (
          <div className="mt-4 pt-4 border-t border-dark-100 animate-in fade-in slide-in-from-top-2 duration-300">
            <Row gutter={[16, 16]}>
              ...
              <Col xs={24} sm={12} md={6}>
                <label className="text-xs font-bold text-dark-400 uppercase tracking-wide mb-1 block">Tipo de Pessoa</label>
                <Select className="w-full" defaultValue="all" options={[
              ...
              <Col xs={24} sm={12} md={8}>
                <label className="text-xs font-bold text-dark-400 uppercase tracking-wide mb-1 block">Recorrência</label>
                <Select className="w-full" defaultValue="all" options={[
                  { label: 'Todas as Recorrências', value: 'all' }, 
                  { label: 'Mensal', value: 'Mensal' }, 
                  { label: 'Anual', value: 'Anual' }, 
                  { label: 'Doação Única', value: 'Única' }
                ]} />
              </Col>
            ...
            <div className="[&_thead_th]:!bg-dark-50 [&_thead_th]:!text-dark-600 [&_thead_th]:!font-bold [&_thead_th]:!py-5 [&_thead_th]:!border-b [&_thead_th]:!border-dark-100">
              <Table 
                ...
                pagination={{ 
                  ...
                  showTotal: (total, range) => `${range[0]}-${range[1]} de ${total} doadores` 
                }} 
            ...
            {mockData.map((donor) => (
              <Col xs={24} sm={12} xl={8} key={donor.id}>
                <Card className="rounded-2xl shadow-soft border-dark-100 hover:shadow-card transition-shadow h-full flex flex-col relative group" bodyStyle={{ padding: '24px' }}>
                  ...
                  {/* Menu de Ações no Modo Grid (Aparece no Hover) */}
                  <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Dropdown menu={{ items: [
                      { key: 'view', label: 'Ver Perfil', icon: <Eye size={16} />, onClick: () => navigate(`/people/donors/${donor.id}`) },
                      { key: 'edit', label: 'Editar', icon: <Edit size={16} />, onClick: () => navigate(`/people/donors/${donor.id}/edit`) },
                      { type: 'divider' },
                      { key: 'delete', label: 'Inativar', icon: <Trash2 size={16} />, danger: true, onClick: () => handleDelete(donor.id, donor.name) },
                    ] }} trigger={['click']}>
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <Avatar src={`https://api.dicebear.com/7.x/initials/svg?seed=${donor.name}&backgroundColor=0047AF`} size={56} className="border border-dark-100" />
                    <Tag className={`rounded-full font-bold uppercase text-[9px] px-2 border m-0 ${donor.status === 'active' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-dark-50 text-dark-400 border-dark-200'}`}>
                  ...
                  <div className="mb-4 pr-6">
                    <h3 className="text-lg font-bold text-dark-900 leading-tight mb-1 flex items-center gap-2">
                      {donor.name}
                      <Tag className="text-[9px] font-bold px-1 py-0 border-dark-200 m-0">{donor.type}</Tag>
                    </h3>
                  ...
                  <div className="mt-auto pt-4 border-t border-dark-50 flex justify-between items-end">
                    <div>
                      <p className="text-[10px] font-bold text-dark-400 uppercase tracking-widest mb-1 flex items-center gap-1"><TrendingUp size={12}/> Total Doado</p>
                      <p className="text-base font-bold text-primary-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(donor.totalDonated)}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] font-bold text-dark-400 uppercase tracking-widest mb-1 flex items-center justify-end gap-1"><Calendar size={12}/> Última</p>
                      <p className="text-sm font-bold text-dark-900">{donor.lastDonation}</p>
                    </div>
                  </div>
                </Card>
              </Col>
