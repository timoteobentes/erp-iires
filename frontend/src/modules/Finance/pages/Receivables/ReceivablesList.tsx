...
const getStatusConfig = (status: string) => ({
  PAID: { text: 'Recebido', classes: 'bg-primary-50 text-primary-600 border-primary-200', icon: <CheckCircle size={12} /> },
...
export default function ReceivablesList() {
  ...
  // --------------------------------------------------------
  // Busca transações da API (apenas INCOME)
  // --------------------------------------------------------
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const all = await transactionsService.list();
      setTransactions(all.filter((t) => t.type === 'INCOME'));
    } catch {
      notification.error({ message: 'Erro', description: 'Não foi possível carregar as receitas.' });
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <strong className="text-dark-900">{title}</strong>?<br />
          O registro será marcado como cancelado.
        </div>
      ),
      okText: 'Sim, cancelar',
      okType: 'danger',
      ...
      onOk: async () => {
        try {
          ...
          notification.success({ message: 'Receita cancelada com sucesso.' });
        } catch {
          notification.error({ message: 'Erro', description: 'Não foi possível cancelar a receita.' });
        }
      },
    });
  };
  ...
  const columns: ColumnsType<Transaction> = [
      ...
      render: (_, record) => (
        <div>
          ...
          <p className="text-xs text-dark-400 font-medium mt-0.5">
            {record.description || record.donor?.name || '—'}
          </p>
        </div>
      ),
    },
    ...
    },
    {
      ...
      align: 'right',
      render: (_, record) => (
        <span className="font-bold text-primary-600">{fmt(record.amount)}</span>
      ),
    },
    {
      title: 'Status',
        ...
        <Space size="small">
          <Tooltip title="Ver Detalhes">
            <Button
              type="text"
              icon={<Eye size={18} />}
              onClick={() => navigate(`/finance/receivables/${record.id}`)}
          ...
          <Tooltip title="Editar">
            <Button
              type="text"
              icon={<Edit size={18} />}
              onClick={() => navigate(`/finance/receivables/${record.id}/edit`)}
      ...
      >
        <div className="[&_thead_th]:!bg-dark-50 [&_thead_th]:!text-dark-600 [&_thead_th]:!font-bold [&_thead_th]:!py-5 [&_thead_th]:!border-b [&_thead_th]:!border-dark-100">
          <Table
            ...
            rowKey="id"
            loading={loading}
            locale={{ emptyText: <Empty description="Nenhuma receita encontrada" image={Empty.PRESENTED_IMAGE_SIMPLE} /> }}
            pagination={{
              pageSize: 10,
              className: 'px-6 py-4 border-t border-dark-100 m-0',
              showTotal: (total, range) => `${range[0]}-${range[1]} de ${total} receitas`,
            }}
